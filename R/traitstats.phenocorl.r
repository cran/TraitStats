traitstats.phenocorl<-function(Treatment,Replication,DataFile)
{
  df_dataset<-DataFile
  ...
  PCR <- matrix( nrow = matrow,ncol = matrow, byrow = TRUE, dimnames = list(rownames, colnames))
  pcr<-c()
  for(i in 1:i)
  {
    for(j in 1:j)
    {
      pcr=c((PV[i,j]/sqrt((PV[i,i])*(PV[j,j]))))
      PCR[i,j]<-pcr
    }
  }
  PhenoCorl<-PCR
