traitstats.genocorl<-function(Treatment,Replication,DataFile)
{
  df_dataset<-DataFile
  ...
  GCR <- matrix( nrow = matrow,ncol = matrow, byrow = TRUE, dimnames = list(rownames, colnames))
  gcr<-c()
  for(i in 1:i)
  {
    for(j in 1:j)
    {
      gcr=c((GV[i,j]/sqrt((GV[i,i])*(GV[j,j]))))
      GCR[i,j]<-gcr
    }
  }
  GenoCorl<-GCR
