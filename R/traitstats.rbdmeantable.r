traitstats.rbdmeantable<-function(Treatment,Replication,DataFile)
{
  df_dataset<-DataFile
  ...
  TrM <- matrix( nrow = t,ncol = matrow, byrow = TRUE, dimnames = list(rownames, colnames))
  incrementor=1
  cc=colcount-2
  for(i in 1:cc)
  {
  ...
  rownames<-c("Trait Mean","SE(m)","CV","CD")
  colnames<-c()
  for(i in 1:matrow)
  {
    colnames <- c(colnames,colnms[i+2])
  }
  MeanTable <- matrix( nrow = 4,ncol = matrow, byrow = TRUE, dimnames = list(rownames, colnames))
  for(i in 1:4)
  {
    for(j in 1:j)
    {
      MeanTable[1,j]<-round(TM[j,j],digits=4)
      MeanTable[2,j]<-round(SEm[j,j],digits=4)
      MeanTable[3,j]<-round(CV[j,j],digits=4)
      MeanTable[4,j]<-round((SEd[j,j]*TINV),digits=4)
    }
  }
  MeanTable
  MeanTable<-rbind(TrM,MeanTable)
  MeanTable
  return(MeanTable)
}
