traitstats.rbdstats<-function(Treatment,Replication,DataFile)
{
  df_dataset<-DataFile
  ...
  rownames<-c("Trait Mean","Min","Max","SE(m)","SE(d)","CV")
  colnames<-c()
  for(i in 1:matrow)
  {
    colnames <- c(colnames,colnms[i+2])
  }
  StatsTable <- matrix( nrow = 6,ncol = matrow, byrow = TRUE, dimnames = list(rownames, colnames))
  for(i in 1:6)
  {
    for(j in 1:j)
    {
      StatsTable[1,j]<-TM[j,j]
      StatsTable[2,j]<-MIN[j,j]
      StatsTable[3,j]<-MAX[j,j]
      StatsTable[4,j]<-SEm[j,j]
      StatsTable[5,j]<-SEd[j,j]
      StatsTable[6,j]<-CV[j,j]
    }
  }
  StatsTable
  return(StatsTable)
